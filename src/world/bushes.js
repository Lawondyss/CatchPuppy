...
export class Bushes {
  ...
  regenerate(player, puppy) {
    this.pool.forEach(bush => { bush.pos = this.k.vec2(-200, -200); });

    if (this.visualBushArea === 0) return;

    // 1. Define a smaller grid cell size to allow for overlapping, creating a maze-like effect.
    const gridCellWidth = this.bushWidth * 0.5;
    const gridCellHeight = this.bushHeight * 0.5;
    ...
    const targetCoverageArea = playableArea * 0.25;
